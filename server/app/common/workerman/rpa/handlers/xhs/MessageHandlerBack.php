...
class MessageHandler extends BaseMessageHandler
{
    ...
    private function _updatePrivateMessage($content)
    {
        ...
        try {
            ...
            //只解析文字消息中的微信号
            $autoStatus = $this->autoAddWechatOperation($content, $account);
            if ($autoStatus) {
            ...
            if ($account->takeover_mode == 1) {
                ...
                if ($this->appType == 1) {
                    ...
                    $reply->voice_enable = $reply->voice_reply_type == 3 ? 0 : 1;
                } else {
                ...
                if ($reply->image_enable == 1 && $request['message_type'] == 2) {
                    if (isset($reply->image_reply_type) && in_array($reply->image_reply_type, [2, 3])) {
    ...
    private function autoAddWechatOperation(array $payload, SvAccount $account)
    {
        try {
            ...
            $wechat = SvAccount::where('device_code', $this->payload['deviceId'])->where('user_id', $account->user_id)->where('type', 1)->limit(1)->findOrEmpty();
            if ($wechat->isEmpty()) {
                $this->setLog('账号未绑定微信账号', 'msg');
                return false;
            }
            $wechat_enable = 1;
            if ($wechat_enable == 1) {
                    ...
                    if (!empty($matchs)) {
                        ...
                        foreach ($matchs as $match) {
                            ...
                            if (!$wechat->isEmpty()) {
                                ...
                                $record['wechat_no'] = $wechat->account;
                                $record['wechat_name'] = $wechat->nickname;
                                $record['status'] = 2;
    ...
    private function sendChannelMessage(array $payload, SvAccount $wechat, array $insertData)
    {
        try {
