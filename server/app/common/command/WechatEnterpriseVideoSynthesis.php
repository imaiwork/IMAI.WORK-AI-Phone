 ...
 * @author dagouzi
 */
class WechatEnterpriseVideoSynthesis extends Command
{
    protected function configure()
    {
        $this->setName('wechat_enterprise_video_synthesis')
            ->setDescription('微信企业自动合成设备视频任务');
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        try {
            ...
            Log::channel('ipVideoSynthesis')->write('微信企业自动合成设备视频任务');
            ...
            foreach ($devices as $device) {
                ...
                if ( Cache::store('redis')->has($cacheKey)) {
                    Log::channel('wechatVideoSynthesis')->write('微信企业设备60分钟内已执行过，跳过：' . $device->device_code);
                    continue;
                }
                // 设置60分钟缓存
                Cache::store('redis')->set($cacheKey, 1, 3600);
                Log::channel('wechatVideoSynthesis')->write('微信企业自动合成设备视频任务：' . $device->device_code);
                try {
                    ...
                    Log::channel('ipVideoSynthesis')->info('设备视频合成异常 device=' . $device->device_code . ' err=' . $e->getMessage());
                }
            }

            return true;
        } catch (\Exception $e) {
            Log::channel('ipVideoSynthesis')->info('微信企业自动合成设备视频任务失败' . $e->getMessage());
            return false;
        } finally {
            print_r("\n 微信企业自动合成设备视频任务结束...'\n");   
