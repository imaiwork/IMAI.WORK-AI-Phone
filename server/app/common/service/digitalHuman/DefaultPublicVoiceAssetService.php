...
class DefaultPublicVoiceAssetService
{
    ...
    {
        return (int)ConfigService::get('default_public_voice', 'assets_synced', 0) === 1;
    }
    ...
    public static function syncToStorage(bool $force = false): array
    {
        ...
        foreach ($files as $relativeUri) {
            ...
            try {
                ...
                Log::channel('digital')->error('默认公共音色母版同步失败: ' . $relativeUri . ' - ' . $e->getMessage());
            }
        }

        if ($failed === []) {
            ConfigService::set('default_public_voice', 'assets_synced', 1);
            ConfigService::set('default_public_voice', 'assets_synced_time', time());
        }
