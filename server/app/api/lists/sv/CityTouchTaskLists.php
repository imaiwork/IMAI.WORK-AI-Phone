...
/**
 * 同城视频截流设置列表
 * Class CityTouchLists
 * @package app\api\lists\sv
 */
class CityTouchTaskLists extends BaseApiDataLists implements ListsSearchInterface
{
    ...
    public function lists(): array
    {
        ...
        return SvCityTouchTaskAccount::alias('ps')
            ...
            ->each(function ($item) {
                ...
                // 请求在线状态
                $detial = SvCityTouchRecord::where('scraping_account_id',$item->id)->where('status', 0)->order('id', 'asc')->limit(1)->find();
                $item['next_CityTouch_time'] = !empty($detial) ? $detial['CityTouch_time'] : '';
                $item['exec_time'] = !empty($detial) ? date('Y-m-d H:i:s', $item['exec_time']) : '';

                $startDate =strtotime($item['CityTouch_start']);
                $endDate = strtotime($item['CityTouch_end']);
                $item['CityTouch_cycle'] = (int)(($endDate - $startDate) / 86400);
    ...
    public function count(): int
    {
        ...
        $this->searchWhere[] = ['ps.media_type', '=', $this->request->get('media_type', 1)];
        $this->searchWhere[] = ['ps.task_type', '=', $this->request->get('task_type', 1)];
        return SvCityTouchTaskAccount::alias('ps')->field('id')
                ->join('sv_account a', 'a.account = ps.account and a.device_code = ps.device_code and a.type = ps.account_type', 'left')
            ->when($this->request->get('start_time') && $this->request->get('end_time'), function ($query) {
                $query->whereBetween('ps.create_time', [strtotime($this->request->get('start_time')), strtotime($this->request->get('end_time'))]);
            })
