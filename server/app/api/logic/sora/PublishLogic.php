...
class PublishLogic extends SvBaseLogic
{
    ...
    public static function add(array $params)
    {
        ...
        try {
            ...
            $params['task_type'] = $params['task_type'] ?? 4;
    ...
    private static function checkPublishTime($params)
    {
        try {
            ...
            $mediaCount = 0;
            if ($params['scene'] == 1) {
                $mediaSettings = SoraVideoSetting::where('id', 'in', $video_ids)->where('user_id', self::$uid)->select()->toArray();
                ...
                $mediaCount = SoraVideoSetting::alias('vs')
                    ->field('vt.*')
                    ->join('sora_video_task vt', 'vs.id = vt.video_setting_id')
        ...
        try {
            ...
            foreach ($accounts as $key => $account) {
                ...
                if (empty($medias) && (int)$account['scene'] === 1) {
                    $settingStatus = SoraVideoSetting::where('id', 'in', json_decode($account['video_ids'], true))->group('status')->column('status');
    ...
    private static function _checkPublishStatus(array $account, array &$usedVideoIds)
    {
        ...
        $settingStatus = SoraVideoSetting::where('id', 'in', $_ids)->group('status')->column('status');
        ...
        if (in_array(0, $settingStatus) || in_array(1, $settingStatus) || in_array(2, $settingStatus)) {
            ...
            if (count($settingStatus) == 1 && array_sum($settingStatus) == 3) {
                ...
                if ($count < $account['count']) {
                    ...
                    $newVideoids = SoraVideoTask::where('video_setting_id', 'in', $_ids)
    ...
    private static function _getMedias(array $account, array &$usedVideoIds)
    {
        try {
            ...
            $status = (int)$account['scene'] === 1 ? [2, 3] : [3];
            $medias = SoraVideoSetting::alias('vs')
                ->field('vt.*')
                ->join('sora_video_task vt', 'vs.id = vt.video_setting_id')
                ->where('vs.id', 'in', $video_ids)
