UPDATE `la_config`
SET `value` = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(`value`, '闪剪', '数字人v2'), '禅境', '数字人v1'), '禅镜', '数字人v1'), '蝉镜', '数字人v1'), '蝉境', '数字人v1')
WHERE (`type` = 'model' AND `name` = 'list')
   OR (`type` = 'digital_human' AND `name` = 'shanjian_auth');
...
SET `channel` = CASE `id`
    ...
    END,
    `configs` = CASE `id`
        WHEN 7 THEN '{"display_aliases":["禅境","禅镜","蝉镜","蝉境","数字人v1"]}'
        WHEN 9 THEN '{"display_aliases":["闪剪","数字人v2","数字人v2"]}'
        ELSE `configs`
    END
WHERE `id` IN (7, 9);

UPDATE `la_models_cost`
SET `channel` = CASE `model_id`
        WHEN 7 THEN 'v1'
        WHEN 9 THEN 'v2'
        ELSE `channel`
    END,
    ...
    `alias` = CASE `model_id`
        ...
        ELSE `alias`
    END
WHERE `model_id` IN (7, 9);

UPDATE `la_model_config`
SET `name` = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(`name`, '闪剪', '数字人v2'), '禅境', '数字人v1'), '禅镜', '数字人v1'), '蝉镜', '数字人v1'), '蝉境', '数字人v1'),
    `description` = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(`description`, '闪剪', '数字人v2'), '禅境', '数字人v1'), '禅镜', '数字人v1'), '蝉镜', '数字人v1'), '蝉境', '数字人v1')
WHERE `scene` LIKE '%chanjing%'
   OR `scene` LIKE '%shanjian%';
