...
SET @s := IF(@c = 0, 'ALTER TABLE `la_user` ADD COLUMN `recharge_stats_reset_time` int(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT ''下级充值业绩统计清零时间点(0=未清零)'' AFTER `total_recharge_amount`', 'DO 0');
PREPARE p FROM @s;
EXECUTE p;
DEALLOCATE PREPARE p;
