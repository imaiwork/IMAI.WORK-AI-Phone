...
import { fetchDeviceAccount } from '@/api/device'
...
export const useDeviceStore = defineStore('device', {
    ...
    getters: {
        ...
        getNextPendingPlatform: (state: DeviceState) => {
            const pendingType = state.platformsToUpdate.find(
                (type) => state.platform[type].status === 0
            )
    ...
    actions: {
        ...
        // 触发 HTTP 拉取账号（进度仍走 WS）
        async sendGetAccountCmd(type: AppTypeEnum) {
            try {
                await fetchDeviceAccount({
                    device_code: this.currentDeviceCode,
                    type
                })
            } catch (error: any) {
                this.setPlatformError(type, error || '下发获取账号指令失败')
                uni.showToast({
                    title: error || '下发获取账号指令失败',
                    icon: 'none',
                    duration: 3000
                })
                if (this.options.isAutoUpdateNextAccount) {
        ...
        // 处理 WebSocket 成功事件（以 appCompleted 为准，账号由服务端落库）
        async handleWebSocketSuccess(data: any) {
            const { type, appType } = data
            if (type !== DeviceCmdEnum.GET_ACCOUNT_INFO_COMPLETE) return
            ...
            if (platformInfo && platformInfo.status === 1) {
                // 多次等待，兼容 appCompleted 早于落库
                for (let i = 0; i < 5; i++) {
                    await new Promise((resolve) => setTimeout(resolve, ACCOUNT_FETCH_REFRESH_DELAY))
                    if (this.options.onAccountsUpdated) {
                        await this.options.onAccountsUpdated()
                    }
                    this.setPlatformAccountSuccess(appType, {
                        account: platformInfo.account,
                        account_no: platformInfo.account_no,
                        avatar: platformInfo.avatar,
                        nickname: platformInfo.nickname,
                        extra: platformInfo.extra
                    })
                    // 若外部 detail 已带回账号则提前结束轮询
                    const detailAccount = this.options.detail?.accounts?.find(
                        (acc: any) => acc.type === appType
                    )
                    if (detailAccount?.account) break
                }
            }
