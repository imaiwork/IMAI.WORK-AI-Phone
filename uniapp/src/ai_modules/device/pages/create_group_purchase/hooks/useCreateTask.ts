...
import { createGroupPurchaseTask, checkTaskPublishTime } from '@/api/device'
import { isCommentUserAction } from './types'
...
export function useCreateTask(formData: PublishFormData) {
    ...
    const buildPayload = () => ({
        ...formData,
        marker_method: formData.marker_method.filter(isCommentUserAction),
    ...
    const executeCreateTask = async () => {
        ...
        try {
            await createGroupPurchaseTask(buildPayload())
            uni.hideLoading()
