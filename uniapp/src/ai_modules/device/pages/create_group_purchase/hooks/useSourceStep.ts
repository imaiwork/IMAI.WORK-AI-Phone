import { getGroupPurchaseKeywordHistory } from '@/api/device'
...
function useKeywordList(
    ...
    options: UseKeywordListOptions
) {
    const input = ref('')
    const expanded = ref(false)

    const visible = computed(() =>
        expanded.value ? getList() : getList().slice(0, KEYWORD_COLLAPSE_THRESHOLD)
    )
...
export function useSourceStep(formData: PublishFormData) {
    ...
    const hasMutexSelected = computed(() =>
        MUTEX_ACTION_LIST.some((a) => formData.marker_method.includes(a.key))
    )
    ...
    const getKeywordHistory = async () => {
        const { filter, nickname_filter } = await getGroupPurchaseKeywordHistory()
