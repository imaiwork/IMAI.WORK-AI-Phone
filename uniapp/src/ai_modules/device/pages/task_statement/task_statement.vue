<template>
    <view class="h-screen flex flex-col bg-[#F2F3F7]">
        <u-navbar :border-bottom="false" :background="{ background: '#FFFFFF' }">
            <view class="ml-[100rpx] mr-[64rpx] w-full flex items-center justify-center">
                <view class="flex items-center gap-[48rpx]">
                    <view
                        v-for="(tab, index) in tabList"
                        :key="index"
                        class="relative flex flex-col items-center pb-[8rpx]"
                        @tap="handleTabChange(tab.type)">
                        <text
                            class="text-[30rpx] font-bold transition-all"
                            :class="currTab == tab.type ? 'text-primary' : 'text-[#999999]'">
                            {{ tab.name }}
                        </text>
                        <view
                            class="absolute bottom-0 h-[4rpx] rounded-full transition-all"
                            :class="currTab == tab.type ? 'bg-primary w-[40rpx]' : 'w-0 bg-[transparent]'">
                        </view>
                    </view>
                </view>
            </view>
            ...
            <scroll-view scroll-x>
                <view class="flex gap-[16rpx] whitespace-nowrap px-[32rpx] py-[20rpx]">
                    <view
                        ...
                        :key="index"
                        class="px-[28rpx] py-[10rpx] rounded-[50rpx] font-semibold transition-all"
                        :class="
                ...
                <view class="px-[32rpx] pt-[24rpx] pb-[40rpx] flex flex-col gap-[32rpx]">
                    <view v-for="(item, index) in dataList" :key="index">
                        ...
                        <view class="bg-white rounded-[20rpx] overflow-hidden">
                            <view class="flex items-center gap-[24rpx] px-[32rpx] py-[28rpx] border-b border-[#F5F5F5]">
                                <image
                                ...
                                <view class="flex-1 min-w-0">
                                    <view class="flex items-center gap-[10rpx] mb-[6rpx]">
                                        <text class="text-[30rpx] font-semibold text-[#1A1A1A] truncate max-w-[240rpx]">
                                ...
                                <view
                                    ...
                                    :class="
                                        item.status == 2 ? 'bg-[#E8F5E9] text-[#2E7D32]' : 'bg-[#FFEBEE] text-[#C62828]'
                                    ">
                                    {{ item.status == 2 ? "成功" : "失败" }}
                                </view>
                            </view>
                            ...
                            <view class="px-[32rpx] py-[24rpx] border-[0] border-t border-solid border-[#F5F5F5]">
                                <view class="flex items-center gap-[12rpx] mb-[16rpx]">
                                    ...
                                    <text class="font-semibold text-[#333333]">任务及状态</text>
                                </view>
                                <view
                                    class="bg-[#FAFAFA] rounded-[12rpx] px-[20rpx] py-[16rpx] flex items-start justify-between gap-[16rpx]">
                                    <text class="flex-1 text-xs font-semibold text-primary leading-relaxed">
                                        {{ item.task_name }}
                                    </text>
                                    <view
                            ...
                            <template v-if="item.task_type == TaskTypeEnum.CLUES">
                                <view
                                    ...
                                    <view class="flex items-center gap-[12rpx]">
                                        ...
                                        <text class="font-semibold text-[#333333]">线索词</text>
                                    </view>
                                    <view class="flex items-center gap-[6rpx] text-[#00000060] text-xs">
                                        共线索词
                                ...
                                <view
                                    ...
                                    <view class="flex items-center gap-[12rpx]">
                                        ...
                                        <text class="font-semibold text-[#333333]">获取线索</text>
                                    </view>
                                    <view class="flex items-center gap-[6rpx] text-[#00000060] text-xs">
                                        共获取线索
                                        <text class="text-[28rpx] font-bold text-primary">{{
                            ...
                            <view
                                ...
                                <view class="flex items-center gap-[12rpx]">
                                    ...
                                    <text class="font-semibold text-[#333333]">私信回复</text>
                                </view>
                                <view class="flex items-center gap-[6rpx] text-[#00000060] text-xs">
                                    私信消息
                                    <text class="text-[28rpx] font-bold text-primary">{{
                            ...
                            <template v-if="item.task_type == TaskTypeEnum.WECHAT_CIRCLE_THUMB_closure">
                                <view
                                    ...
                                    <view class="flex items-center gap-[12rpx]">
                                        ...
                                        <text class="font-semibold text-[#333333]">{{
                                    ...
                                    <view
                                        class="flex items-center gap-[6rpx] text-[#00000060] text-xs"
                            ...
                            <view
                                ...
                                <view class="flex items-center gap-[12rpx]">
                                    ...
                                    <text class="font-semibold text-[#333333]">自动加微</text>
                                </view>
                                <view
                                    class="flex items-center gap-[6rpx] text-[#00000060] text-xs"
                            ...
                            <template v-if="item.task_type == TaskTypeEnum.TOUCH">
                                <view
                                    ...
                                    <view class="flex items-center gap-[12rpx]">
                                        ...
                                        <text class="font-semibold text-[#333333]">评论区回复</text>
                                    </view>
                                    <view
                                        class="flex items-center gap-[6rpx] text-[#00000060] text-xs"
                                ...
                                <view
                                    ...
                                    <view class="flex items-center gap-[12rpx]">
                                        ...
                                        <text class="font-semibold text-[#333333]">评论区私信</text>
                                    </view>
                                    <view
                                        class="flex items-center gap-[6rpx] text-[#00000060] text-xs"
                                        @click.stop="handleTaskStatementDetail(item, 'closure')">
                                ...
                                <view
                                    ...
                                    <view class="flex items-center gap-[12rpx]">
                                        ...
                                        <text class="font-semibold text-[#333333]">关注/点赞</text>
                                    </view>
                                    <view
                                        class="flex items-center gap-[6rpx] text-[#00000060] text-xs"
                                        @click.stop="handleTaskStatementDetail(item, 'closure')">
        ...
        <template #content>
            <view class="h-full flex flex-col py-[32rpx]">
                <view
                    ...
                    <text class="font-semibold text-[#333333]">算力总消耗：</text>
...
const handleTaskStatementDetail = (
    ...
    uni.$u.route({
        ...
        params: {
            task_id: item.id,
            sub_id: item.sub_task_id,
            type,
            data_info: JSON.stringify(item.data_info),
        },
    });
};
...
onLoad((options: any) => {
    ...
    if (options.type) currTab.value = Number(options.type);
});
</script>
