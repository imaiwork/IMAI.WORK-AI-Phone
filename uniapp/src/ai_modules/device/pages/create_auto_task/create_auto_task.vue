<template>
    ...
    <view class="grow min-h-0">
      ...
      >
        <view class="p-4 pb-[350rpx]">
          <view class="mb-6">
            <text class="text-[34rpx] font-medium text-primary"
              >完善运营方案</text
            >
          ...
          >
            <view class="flex flex-col mb-3">
              ...
              >
              <text class="text-[24rpx] text-[#94A3B8] mt-1">{{
                item.subLabel
              }}</text>
            </view>

            <view
              class="bg-white rounded-[24rpx] px-4 py-2 border border-[#F1F5F9]"
            >
              <u-input
                type="textarea"
        ...
        <view class="p-4">
            ...
            <view class="bg-white rounded-lg p-4 flex flex-col gap-y-2">
              ...
              <template v-if="!isAnalyzingFinished">
                ...
                <view class="flex gap-x-2">
                  ...
                  ></image>
                  <view
                    class="text-[#000000]/90 font-medium"
                    v-if="isAnalyzingFinished"
                  >
                    已完成解析
                  </view>
                  <template v-else>
          ...
          <view class="mt-4 space-y-3" v-if="isAnalyzingFinished">
            <view class="bg-white rounded-lg px-4 pt-3 pb-6">
              ...
              <view class="text-[#000000]/70 mt-2">
                自动化才是真AI，傻瓜化才是强AI
              </view>
              <view class="mt-4 text-[#000000]/70 leading-[1.7]">
                老板，以下是我给咱公司出的AI获客解决方案。本方案基于系统<text
                  class="text-primary"
                  >24小时自动执行任务</text
              ...
              <view class="mt-4 rounded-[10rpx] bg-[#f6f6f6] flex p-3 gap-x-3">
                <view
                  class="w-[6rpx] bg-[#DADEE5] rounded-[10rpx] shrink-0"
                ></view>
                <view>
                  咱们的客户是：<text class="font-medium">{{
                    reportData.industryType || "--"
                  }}</text
                  >的<text class="font-medium">
                    {{ reportData.targetAudience || "--" }} </text
              ...
              <view class="mt-[46rpx]">
                ...
                <view class="relative mt-[38rpx]">
                  <image
                    ...
                    class="w-[50rpx] h-[42rpx] absolute top-[-15rpx]"
                  ></image>
                  ...
                  <image
                    ...
                    class="w-[50rpx] h-[42rpx] absolute bottom-[-15rpx] right-8"
                  ></image>
                </view>
              </view>
            </view>
            <view class="bg-white rounded-lg px-4 pt-3 pb-6">
              <view class="leading-[1.7]">
                系统会自动搭建<text class="relative"
                  >4个IP人设矩阵<text
                    class="absolute left-0 bottom-0 opacity-60 h-[8rpx] w-full bg-[#BAF7E7]"
                  ></text> </text
                ...
                <view class="flex items-center gap-x-2">
                  ...
                  <view class="flex items-center gap-x-1 font-medium">
                    ...
                    <text class="text-primary">{{
                      reportData.mainPlatform || "--"
                    }}</text>
                  </view>
                </view>
                <view class="p-3 rounded-[20rpx] bg-[#F5F5FE] mt-3">
                  <text class="text-[#000000]/50">原因：</text
                  >{{ reportData.platformLogic || "--" }}
                </view>
              </view>
              <view class="mt-[60rpx]">
                <view class="flex items-center gap-x-2">
                ...
                <view class="mt-3 space-y-2">
                  <view
                    class="p-3 rounded-[20rpx] bg-[#F5F5FE] mt-3 flex items-center gap-x-2"
                  >
                  ...
                  <view
                    class="p-3 rounded-[20rpx] bg-[#FEF4E3] mt-3 flex items-center gap-x-2"
                  >
                    {{ reportData.auxiliaryPlatform2 || "--" }}：{{
                      reportData.ipContent || "--"
                    }}
                  </view>
                  <view
                    class="p-3 rounded-[20rpx] bg-[#F3FAF2] mt-3 flex items-center gap-x-2"
                  >
            ...
            <view class="bg-white rounded-lg px-3 pt-3 pb-6">
              <view class="flex gap-x-2">
                ...
                <view class="font-medium text-[34rpx]">
                  系统每天固定<text class="relative"
                    >"三轮内容发布"<view
                      class="absolute left-0 bottom-[2rpx] opacity-80 h-[8rpx] w-full bg-[#BAF7E7] rounded-[16rpx]"
                    ></view
                  ></text>
                </view>
              </view>
              <view class="mt-[46rpx] px-2 space-y-5">
                <view class="flex gap-x-3 relative">
                  <view
                    class="absolute w-[2rpx] h-[60rpx] bg-[#E5E5E5] top-[50rpx] left-[8rpx]"
                  ></view>
                  <view
                  ...
                  <view>
                    <view class="text-[34rpx] text-primary text-medium">
                      08:00 - 08:30
                    </view>
                    <view class="mt-1">
                      {{ reportData.industryContent || "--" }}
              ...
              <view
                class="px-[42rpx] py-[32rpx] bg-[#F5F5FE] mt-[50rpx] rounded-[20rpx]"
              >
                每天<text class="text-primary">4个</text>平台总计<text
                  class="text-primary"
                  >12条</text
                >内容自动产出， 一个月稳定输出约<text class="text-primary"
                  >360条</text
                >内容。
              </view>
              <view class="mt-8 h-[1rpx] w-full bg-[#000000]/5"></view>
              <view class="flex gap-x-2 mt-[60rpx]">
                ...
                <view class="font-medium text-[34rpx]">
                  ...
                  <view class="relative w-fit"
                    >"回复私信信息"<view
                      class="absolute left-0 bottom-[4rpx] opacity-60 h-[8rpx] w-full bg-[#BAF7E7] rounded-[16rpx]"
                    ></view
                  ></view>
                </view>
              </view>
              ...
              <view class="mt-[48rpx] space-y-2">
                <view
                  class="px-3 py-3 rounded-[20rpx] bg-[#F5F5FE] mt-3 flex items-center gap-x-2"
                >
                  <image
                ...
                >
                  <image
                    src="/static/images/common/redbook_s.png"
                    class="w-[40rpx] h-[40rpx]"
                  ></image
                  >小红书：约 10条
                </view>
                <view
                  class="px-3 py-3 rounded-[20rpx] bg-[#FEF4E3] mt-3 flex items-center gap-x-2"
                >
                  <image
                    src="/static/images/common/sph_s.png"
                    class="w-[40rpx] h-[40rpx]"
                  ></image
                  >视频号：点赞约 50个
                </view>
                <view
                  class="px-3 py-3 rounded-[20rpx] bg-[#F3FAF2] mt-3 flex items-center gap-x-2"
                >
                  <image
              ...
              <view class="mt-[44rpx]">
                合计每天至少<text class="text-primary">120次</text
                >精准互动触达。
              </view>
            </view>
            <view class="bg-white rounded-lg px-3 pt-3 pb-6">
            ...
            <view class="mt-[48rpx] px-6">
              接下来，您只需要上传素材，其他的交给我就好。
            </view>
          </view>
        </view>
      </scroll-view>
            ...
            <view>
              ...
              <view class="rounded-[20rpx] bg-white p-[30rpx] mt-[18rpx]">
                <view
                  ...
                  class="grid grid-cols-3 gap-x-[20rpx]"
                >
                  <view
                ...
                <view
                  v-else
                  class="flex flex-col items-center justify-center gap-y-[20rpx] py-4"
                >
                  <view class="text-center text-[#0000004d]"
                    >你还没有添加数字人形象</view
                  >
                  <view
      ...
      <view class="p-4">
        <view class="flex items-center gap-4" v-if="step === 1">
          ...
          >
            ...
            <text
              class="text-primary text-[32rpx] font-black tracking-wide ml-2"
              >点击说话</text
            >
          </view>
          <view
            ...
            @click="startAnalysis"
          >
            <text class="text-white text-[32rpx] font-black tracking-wide ml-2"
              >开始分析</text
            >
          </view>
        </view>
  ...
  <confirm-dialog
    ...
    @confirm="handleConfirmCreateSuccess"
  ></confirm-dialog>
...
const formConfigs = ref<AnalysisStep[]>([
  ...
  },
  {
    ...
    label: "IP人设风格",
    subLabel:
      "从专业权威、亲切邻家、幽默搞笑、热血励志、精致美学、真实接地气中选1-3个关键词，或补充其他风格",
    status: 0,
  },
  {
    key: "targetCustomers",
  ...
  },
  {
    ...
    label: "内容偏好",
    subLabel:
      "倾向于口播、剧情、知识分享、幕后记录、客户见证等哪种形式？是否有不愿尝试的形式？",
    status: 0,
  },
  {
...
const startAnalysis = async () => {
  ...
  // 验证表单
  const formDataItem = formConfigs.value.find(
    (item: any) => formData[item.key] === "",
  );
  if (formDataItem) {
  ...
  const showStep = async (index: number, duration = 800) => {
    ...
    await new Promise(
      (resolve) => (analysisTimer.value = setTimeout(resolve, duration)),
    );
...
const handleConfirmCreateSuccess = () => {
  uni.$u.route({ url: "/ai_modules/device/pages/auto_task/auto_task", type: "reLaunch", params: { device_code: deviceCode.value } });
};
