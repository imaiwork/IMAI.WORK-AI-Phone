<template>
  <view class="h-screen device-bg flex flex-col">
      ...
      <view class="grid grid-cols-4 w-full px-4">
        ...
        >
          ...
          <view
            ...
            :class="{ '!border-primary': step > item.step }"
          ></view>
        </view>
      </view>
    </view>
      ...
      <scroll-view class="h-full" scroll-y v-show="step === 1">
        <view class="px-4 pb-[100rpx]">
          <view class="px-[40rpx] py-[30rpx] bg-white rounded-[20rpx]">
            ...
            <view class="mt-[36rpx]" v-if="formData.customer_type === 0">
              ...
              >
                <view
                  class="font-medium"
                  :class="{ 'text-primary': formData.region }"
                >
                  {{ formData.region || "无限制" }}
                </view>
                <view
                  class="flex items-center gap-x-1 text-primary font-medium"
                >
                  修改
          ...
          <view class="mt-[30rpx]" v-if="formData.customer_type === 0">
            ...
            <view
              class="mt-[20rpx] bg-white rounded-[20rpx] px-[40rpx] py-[30rpx]"
            >
              <view class="flex items-center gap-x-2">
                <view
                  class="flex-1 bg-[#F3F3F3] rounded-[10rpx] h-[80rpx] flex items-center px-4"
                >
                  <u-input
                    class="w-full"
              ...
              <view class="mt-[30rpx] bg-[#F3F3F3] rounded-[16rpx]">
                ...
                <view
                  v-else
                  class="px-[24rpx] py-[50rpx] text-[#0000004d] text-center"
                >
                  输入或AI生成获客行业
                </view>
                <view class="p-[24rpx]">
              ...
              <view class="mt-[30rpx]" v-if="historyIndustry.length > 0">
                <view class="flex items-center justify-between">
                  ...
                  <view
                    class="font-medium"
                    @click="showHistoryIndustryPopup = true"
                  >
                    更多
                    <u-icon
                      name="arrow-right"
      ...
      <scroll-view class="h-full" scroll-y v-show="step === 2">
        <view class="px-4 pb-[100rpx]">
          ...
          <view class="mt-[50rpx]" v-if="!isCollect">
            ...
            <view
              class="mt-[18rpx] rounded-[20rpx] bg-white px-[30rpx] py-[28rpx]"
            >
          ...
          <view class="mt-[50rpx]" v-if="isCollect">
            ...
            >
              ...
              <template v-if="isPinLunWork">
                ...
                <view class="font-medium text-primary mb-[30rpx]"
                  >评论作品方式</view
                >
                ...
                <view class="mt-[28rpx]" v-if="formData.comment_type === 1">
                  <view
                    class="bg-[#F3F4FB] rounded-[16rpx] px-[30rpx] py-[28rpx]"
                    v-if="formData.fixed_comment_list.length > 0"
                  >
                    ...
                    <view class="flex flex-wrap gap-2">
                      ...
                      >
                        <text class="line-clamp-2">{{ item }}</text>
                        <view
                          class="flex-shrink-0 rounded-full flex item-center justify-center w-4 h-4 bg-[#0000004C]"
                  ...
                  <view v-else class="mt-4">
                    ...
                    >
                      <u-icon name="plus" size="20"></u-icon>
                      <text class="font-medium ml-1">添加固定话术</text>
                    </view>
                  </view>
                </view>
              </template>
            ...
            >
              ...
              <view class="flex items-center gap-x-1">
                <text class="line-clamp-1 break-all text-primary font-medium">{{
                  formData.comment_region.length > 0
                    ? formData.comment_region.join(";")
                    : "不限"
                }}</text>
                <u-icon name="arrow-right" size="22" color="#B2B2B2"></u-icon>
              </view>
    ...
    <view
      class="bg-white shadow-[0_0_0_1rpx_rgba(0,0,0,0.05)] flex-shrink-0 pb-5"
    >
      <view
        class="flex items-center px-4 h-[140rpx]"
  ...
  />
  <comment-filter
    v-model="showCommentFilterEdit"
    @confirm="handleCommentFilterConfirm"
  />
  <confirm-dialog
  ...
  />
  <choose-region
    v-model="showChooseRegionPopup"
    @confirm="handleChooseRegionConfirm"
  />
  <choose-age
    ref="chooseAgeRef"
  ...
  <choose-comment-time
    ...
    :value="
      commentTimeType === 'content'
        ? formData.content_time_index
...
const keywordsEditType = ref<
  "clue" | "comment" | "comment_content" | "fixed_comment"
>("clue");
...
const isCollect = computed(
  () => createType.value === CreateTypeEnum.COLLECT_MARKETING,
);
...
const isPinLunWork = computed(() => {
  return getTouchTypeList.value.some(
    (item: any) => item.checked && item.name == 4,
  );
});

function useNumSelection(
  list: number[],
  initialVal: number,
  title: string,
...
const canStepProceed = (stepNumber: number): boolean => {
  switch (stepNumber) {
    ...
    case 2:
      if (isCollect.value) {
        const hasSelectedTouchType = getTouchTypeList.value.some(
          (item: any) => item.checked,
        );
        if (!hasSelectedTouchType) return false;

        const hasCommentWork = getTouchTypeList.value
...
const getTimeLabel = (type: "content" | "comment"): string => {
  if (type === "content") {
    return formData.content_time > -1
      ? commentTimeList[formData.content_time_index[0]].label
      : "不限";
  } else {
    return formData.comment_time > -1
      ? commentTimeList[formData.comment_time_index[0]].label
      : "不限";
  }
};
...
const handleFixedCommentEdit = (index: number) => {
  ...
  showKeywordsEdit.value = true;
  if (index > -1) {
    keywordsEditRef.value?.setFormData(formData.fixed_comment_list[index]);
  }
};

const handleFixedCommentDelete = (index: number) => {
  formData.fixed_comment_list.splice(index, 1);
};
...
const handleKeywordsEditConfirm = (data: any) => {
  ...
  } else if (keywordsEditType.value === "fixed_comment") {
    if (keywordsEditIndex.value === -1) {
      formData.fixed_comment_list.push(data);
    } else {
      formData.fixed_comment_list[keywordsEditIndex.value] = data;
    }
  }
...
const handleCommentTimeConfirm = (res: any) => {
  if (commentTimeType.value === "content") {
    ...
    formData.content_time =
      commentTimeList[formData.content_time_index[0]].value;
  } else {
    formData.comment_time_index = res;
    formData.comment_time =
      commentTimeList[formData.comment_time_index[0]].value;
  }
};
...
const handleCreateTask = async () => {
  ...
  try {
    const params = {
      ...
      marker_method: getTouchTypeList.value
        .filter((item) => item.checked)
        .map((item) => item.name),
    };
...
onLoad((options: any) => {
  ...
  formData.name = `${
    isCollect.value ? "留痕" : "截流"
  }获客任务${uni.$u.timeFormat(new Date(), "yyyymmddhhMM")}`;
  // 事件监听
  on("confirm", (res: any) => {
    const { type, data } = res;
