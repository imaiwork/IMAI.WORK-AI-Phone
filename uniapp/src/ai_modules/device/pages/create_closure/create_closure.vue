                    ...
                    <view v-if="formData.customer_type === 0">
                        ...
                        <view
                            ...
                            <view class="flex items-center justify-between">
                                ...
                                <view class="text-primary font-medium" @click="handleCommentFilterClear">清空</view>
                            </view>
                    ...
                    <view class="mt-[50rpx]" v-if="!isCollect">
                        ...
                        <view class="mt-[18rpx] rounded-[20rpx] bg-white px-[30rpx] py-[28rpx]">
                            <view class="flex items-center justify-between">
                                ...
                                <view class="text-primary font-medium" @click="handleCommentTypeClear">清空</view>
                            </view>
...
const isPinLunWork = computed(() => {
    return getTouchTypeList.value.some((item: any) => item.checked && item.name == 4);
});
...
const canStepProceed = (stepNumber: number): boolean => {
    switch (stepNumber) {
        ...
        case 2:
            if (isCollect.value) {
                const hasSelectedTouchType = getTouchTypeList.value.some((item: any) => item.checked);
                ...
                const hasCommentWork = getTouchTypeList.value
                    ...
                    .map((item) => item.name)
                    .includes(4);
                if (hasCommentWork && formData.comment_type === 1 && formData.fixed_comment_list.length === 0) {
                    return false;
                }
                return true;
            } else {
...
const handleCommentFilterEdit = (index: number) => {
    ...
    showKeywordsEdit.value = true;
    keywordsEditRef.value?.setFormData(index > -1 ? formData.comment_filter_list[index].value : "");
};
