<template>
  <view class="auto-task-page" v-if="!loading">
    ...
    >
      <view class="px-[42rpx] flex items-center justify-between">
        ...
        <view class="flex items-center gap-x-1">
          ...
          <view v-else class="text-[#FF2442] text-xs font-medium"
            >无法执行，配置未完整</view
          >
        </view>
      </view>
            ...
            >
              ...
              >
                ...
                <view class="flex flex-col gap-y-[4rpx]">
                  <view
                    class="w-[5rpx] h-[8rpx] rounded-[50rpx] bg-[#0000004d]"
                  ></view>
                  <view
                    class="w-[5rpx] h-[10rpx] rounded-[50rpx] bg-[#0000004d]"
                  ></view>
                  <view
                    class="w-[5rpx] h-[8rpx] rounded-[50rpx] bg-[#0000004d]"
                  ></view>
              ...
              <view
                class="bg-white flex-1 relative rounded-[20rpx] px-[40rpx] py-[30rpx]"
              >
                <view
                ...
                <view class="flex items-center justify-between mt-2">
                  <view class="flex items-center gap-x-1">
                    ...
                    ></image>
                    <text class="text-xs text-[#0000004d] font-medium"
                      >任务类型</text
                    >
                  </view>
                </view>
                ...
                <view class="flex items-center justify-between">
                  ...
                  >
                    ...
                    <template v-else>
                      <template v-if="item.status == 1 || !isCompleteConfig">
                        已配置
                      </template>
    ...
    <view
      class="absolute h-full bottom-0 left-0 w-full z-[88] flex flex-col justify-end"
      v-if="!isCompleteConfig"
    >
      <view
        class="h-full w-full pt-[200rpx]"
        style="
          background: linear-gradient(360deg, #f3f4fb 30%, transparent 100%);
        "
      >
        <view class="flex flex-col items-center absolute bottom-[15vh] w-full">
          <view class="font-medium text-primary text-[40rpx]">
            请完善首次设置
          </view>
          <view
          ...
          >
          <view
            class="mt-10 flex items-center gap-x-1"
            @click="showFeePopup = true"
          >
  ...
  <u-popup v-model="showChooseApp" mode="center" border-radius="20" width="80%">
    <view class="bg-white p-6 rounded-2xl">
      ...
      <text class="text-sm text-[#9ca3af] mb-5 block"
        >请选择您要发布的平台</text
      >
      ...
      <view class="space-y-3">
        ...
        >
          ...
          >
            <image
              :src="platform.activeIcon"
              class="w-[48rpx] h-[48rpx]"
            ></image>
          </view>
          <view class="flex-1">
            <text class="font-semibold text-base block">{{
              platform.name
            }}</text>
            <text class="text-xs text-[#9ca3af]" v-if="platform.desc">{{
              platform.desc
            }}</text>
          </view>
          <view
          ...
          >
            <view class="flex items-center gap-3 flex-1 mr-4">
              <view class="flex flex-col">
                <text class="text-[30rpx] font-medium text-slate-800 mb-1">{{
                  item.name
                }}</text>
                <text
                  class="text-[24rpx] text-slate-400 leading-snug line-clamp-1"
                >
                  {{ item.description }}
                </text>
              </view>
...
const toTaskConfig = (item: any) => {
  ...
  // 3. 原有逻辑：URL映射和页面跳转
  const urls: any = {
    [TaskKeyEnum.CLUES_SETTING]:
      "/ai_modules/device/pages/setting_clue/setting_clue",
    [TaskKeyEnum.PUBLISH_SETTING]:
      "/ai_modules/device/pages/setting_publish/setting_publish",
    [TaskKeyEnum.TAKEOVER_SETTING]:
      "/ai_modules/device/pages/setting_private_take/setting_private_take",
    [TaskKeyEnum.CIRCLE_INTERACTION]:
      "/ai_modules/device/pages/setting_circle/setting_circle",
    [TaskKeyEnum.TOUCH_SETTING]:
      "/ai_modules/device/pages/setting_ca/setting_ca",
    [TaskKeyEnum.ADD_WECHAT_SETTING]:
      "/ai_modules/device/pages/setting_add_wechat/setting_add_wechat",
  };
