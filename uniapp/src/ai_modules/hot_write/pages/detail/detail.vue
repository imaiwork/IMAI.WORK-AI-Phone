...
import { getHotWriteDetail, generateVideo, confirmPublishText, createHotWrite } from "@/api/hot_write";
...
import { HotWriteTaskStatus, isImageTextTask } from "@/ai_modules/hot_write/enums";
...
const handleRetryVideo = async () => {
    ...
    const personaId = taskDetail.value?.persona_id;
    if (!url) {
    ...
    try {
        await createHotWrite({
            ...
            visual_material_source: taskDetail.value.visual_material_source ?? 3,
        });
        uni.hideLoading();
...
onHide(() => endImageTextPoll());
onUnload(() => endImageTextPoll());
onUnmounted(() => endImageTextPoll());
</script>
