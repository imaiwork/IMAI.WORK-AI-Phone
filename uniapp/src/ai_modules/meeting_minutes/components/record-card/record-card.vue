<template>
    <view class="bg-white rounded-2xl border border-solid border-[#e8eaf2]" @click="handleItem(item)">
        <view class="flex flex-col items-center">
            <view class="rounded-2xl border-[8rpx] border-solid border-white w-full h-auto">
                <view class="content-box">
                    <view v-if="item.status == TurnStatus.SUCCESS" class="absolute inset-0">
                        <template v-if="getResult(item)">
                            ...
                            <view class="absolute top-0 left-0 pt-[32rpx] px-[48rpx] pb-[28rpx] z-2 w-full h-full">
                                <view
                ...
                <view class="w-full h-[132rpx] py-3 px-4">
                    <view class="text-ellipsis whitespace-nowrap overflow-hidden font-semibold">
                    ...
                    <view class="mt-2">
                        <template v-if="item.status == TurnStatus.SUCCESS">
                            <view
                                ...
                                v-if="getTags(item) && getTags(item).length > 0">
                                <view
                            ...
                            <view class="text-xs text-[#8f91a8]" v-else> 内容为空 </view>
                        </template>
                        <view
                ...
                <view class="px-[20rpx] pb-[20rpx] flex items-center justify-between gap-2">
                    <view class="flex items-center gap-2 justify-end grow">
                        ...
                        <view class="px-2 py-1" @click.stop="handleMore">
                            <u-icon name="more-dot-fill" size="28" color="#B2B5B6"></u-icon>
                        </view>
                    </view>
                </view>
            </view>
...
const { handleAgain, handleDelete, formatName, formatDate, handleItem, getTags, getDuration, getResult } =
    useHandleApi();
...
.time-container {
    background: linear-gradient(117.59deg, #5f90d9 0%, #94c6ff 38.9%, #6eb0f8 74.98%, #71a5f8 100%);
