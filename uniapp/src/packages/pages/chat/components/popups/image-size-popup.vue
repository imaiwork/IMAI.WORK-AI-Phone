        ...
        <template #header>
            <view class="px-[40rpx] pt-3 pb-[24rpx] border-b border-solid border-[#F3F4F6]">
                ...
                <view class="flex items-center justify-between">
                    <view class="text-[32rpx] font-bold text-[#1F2937]">画面尺寸</view>
                    <view
                        ...
                        <view class="px-[32rpx] py-[24rpx]">
                            ...
                            <scroll-view scroll-x class="w-full">
                                <view class="flex items-end gap-x-[28rpx] pb-[8rpx]">
                                    <view
                                        v-for="r in IMAGE_RATIO_OPTIONS"
                                        :key="r.key"
                            ...
                            <view class="flex gap-x-[20rpx]">
                                <view
                                    v-for="opt in IMAGE_RESOLUTION_OPTIONS"
                                    :key="opt.key"
                                    class="res-btn"
                            ...
                            <!-- seedream4.0 等 maxCount=1 时不展示张数（对齐 PC） -->
                            <view
                                v-if="maxCount > 1"
                                class="flex items-center justify-between mt-[40rpx] mb-[12rpx]">
                                <view class="text-[26rpx] font-semibold text-[#6B7280]">生成张数</view>
                                <view class="flex items-center gap-x-[24rpx]">
                                    <view
                                        class="step-btn"
                                        :class="{ 'opacity-35': draftCount <= 1 }"
                                        @click="stepCount(-1)">
                                        −
                                    </view>
                                    <text class="min-w-[40rpx] text-center text-[30rpx] font-bold text-[#111827]">
                                        {{ draftCount }}
                                    </text>
                                    <view
                                        class="step-btn"
                                        :class="{ 'opacity-35': draftCount >= maxCount }"
                                        @click="stepCount(1)">
                                        ＋
                                    </view>
                                </view>
                            </view>
                            <view v-else class="mt-[40rpx] text-[24rpx] text-[#9CA3AF]">
                                当前模型仅支持生成 1 张
                            </view>
                        </view>
                    </scroll-view>
...
import { IMAGE_RATIO_OPTIONS, IMAGE_RESOLUTION_OPTIONS } from "../../enums/workbench";
...
const emit = defineEmits<{
    ...
    (e: "confirm", payload: { ratio: string; resolution: string; count: number }): void;
}>();
...
const draftCount = ref(props.count);
...
watch(
    ...
    (v) => {
        ...
        draftCount.value = Math.max(1, Math.min(props.maxCount, Number(props.count) || 1));
    },
);

const stepCount = (delta: number) => {
    draftCount.value = Math.max(1, Math.min(props.maxCount, draftCount.value + delta));
};
...
const onConfirm = () => {
    emit("confirm", {
        ...
        count: Math.max(1, Math.min(props.maxCount, draftCount.value)),
    });
...
.step-btn {
    @apply w-[64rpx] h-[64rpx] rounded-[16rpx] bg-[#F3F4F6] flex items-center justify-center text-[36rpx] text-[#374151];
}
</style>
