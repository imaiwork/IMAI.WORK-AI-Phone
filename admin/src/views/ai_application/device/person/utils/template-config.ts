...
export const getSynthApiShortLabel = (apiType: number): string =>
    SYNTH_API_SHORT_LABEL[apiType] || String(apiType);

export const getSynthApiFullLabel = (apiType: number): string =>
    SYNTH_API_FULL_LABEL[apiType] || String(apiType);
...
export const normalizeTemplateItem = (
    apiType: number,
    raw?: Partial<TemplateConfigItem> | null,
): TemplateConfigItem => {
...
/** 自定义模式下是否存在未勾选模板的类型 */
export const findEmptyCustomType = (apiTypes: number[], config: TemplateConfigMap): number | null => {
