<template>
    <div>
        ...
        <el-card class="!border-none mt-4" shadow="never">
            <div class="mb-4 flex justify-between">
                <el-button
                    v-perms="['ai_application.montage.role/delete']"
                    type="default"
                    :plain="true"
            ...
            <el-table
                ...
                <el-table-column label="任务状态" min-width="120">
                    <template #default="{ row }">
                        ...
                        <template v-else>
                            <el-tag type="info">生成中</el-tag>
                        </template>
                    </template>
                </el-table-column>
                <el-table-column label="是否公开" width="140" v-perms="['ai_application.montage.role/setConfig']">
                    <template #default="{ row }">
                        <el-switch
                            v-model="row.is_public"
                            :active-value="1"
                            :inactive-value="0"
                            @change="handleIsPublic(row)" />
                    </template>
                </el-table-column>
                <el-table-column label="消耗算力" prop="token" min-width="120">
                    <template #default="{ row }">
                        <div>{{ row.token || 0 }}算力</div>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="120" fixed="right">
                    <template #default="{ row }">
                        <el-button type="primary" link @click="handlePlay(row.anchor_url)"> 播放 </el-button>
                        <el-button
                            v-perms="['ai_application.montage.role/delete']"
                            type="danger"
                            link
    ...
    <el-dialog v-model="showVideo" width="740px" title="视频预览">
        <div>
            <div class="text-lg font-bold mb-2">角色视频</div>
...
import {
    getMontageSoraRoleList,
    deleteMontageSoraRole,
    updateMontageSoraRole,
...
const { pager, getLists, resetPage, resetParams } = usePaging({
    fetchFun: getMontageSoraRoleList,
    params: queryParams,
});
...
const handleDelete = async (id: number | number[]) => {
    ...
    await deleteMontageSoraRole({ id });
    getLists();
    multipleSelection.value = [];
    tableRef.value?.clearSelection();
};

const handleIsPublic = async (row: any) => {
    try {
        await updateMontageSoraRole({ id: row.id, is_public: row.is_public });
        getLists();
    } catch (error) {}
};
