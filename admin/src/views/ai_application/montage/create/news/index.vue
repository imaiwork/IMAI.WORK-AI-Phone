    ...
    <div>
        ...
        <el-card class="!border-none mt-4" shadow="never">
            <div class="mb-4 flex justify-between">
                <el-button
                    v-perms="['ai_application.montage.create.news/delete']"
                    type="default"
                    :plain="true"
            ...
            <el-table
                ...
                <el-table-column label="素材组数" prop="material_num" width="100" />
                <el-table-column label="素材总数量" prop="name" width="120">
                    <template #default="{ row }">
                        <span>{{ JSON.parse(row.material)?.length || 0 }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="文案数量" prop="name" width="100">
                    <template #default="{ row }">
                        <span>{{ JSON.parse(row.copywriting)?.length || 0 }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="创作数量" prop="video_count" width="100" />
                ...
                <el-table-column label="操作" width="120" fixed="right">
                    <template #default="{ row }">
                        <el-button v-perms="['ai_application.montage.create.news/detail']" type="primary" link>
                            <router-link
                                :to="{
                                    path: getRoutePath('ai_application.montage.create.news/detail'),
                                    query: {
                                        id: row.id,
                                        name: row.name,
                                    },
