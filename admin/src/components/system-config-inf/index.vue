      ...
      >
        ...
        >
          <div v-if="!isHovered" class="absolute inset-0">
            <div
              class="absolute inset-0 rounded-full border-2 border-[#f97316] opacity-60 animate-ripple-1"
            ></div>
            <div
              class="absolute inset-0 rounded-full border-2 border-[#f97316] opacity-40 animate-ripple-2"
            ></div>
            <div
              class="absolute inset-0 rounded-full border-2 border-[#f97316] opacity-20 animate-ripple-3"
            ></div>
          </div>
          ...
          <div class="relative w-full h-full flex items-center">
            ...
            <div
              v-if="isHovered"
              class="w-full h-full flex items-center px-4 space-x-3"
            >
      ...
      <div v-if="isModalOpen" class="fixed inset-0 z-[99999] backdrop-blur-sm bg-opacity-50">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
          ...
          >
            ...
            >
              <div
                class="flex items-center justify-between px-6 py-4 border-b border-[#e5e7eb]"
              >
                <div class="flex items-center space-x-3">
                  <el-icon class="text-xl text-[#6b7280]">
                    <Setting />
                  </el-icon>
                  <h3 class="text-lg font-medium text-[#111827]">
                    系统配置检查
                  </h3>
                </div>
                <button
                ...
                <div class="px-6 py-4">
                  <div class="space-y-3">
                    ...
                    >
                      ...
                      <el-button
                        type="primary"
                        @click="handleGoConfig(item.routeAuth)"
                        >前往配置</el-button
                      >
                    </div>
                  </div>
...
// 计算属性
const firstUnconfigured = computed(() =>
  systemConfig.value.find((item: any) => item.status === 0),
);

const unconfiguredCount = computed(
  () => systemConfig.value.filter((item: any) => item.status === 0).length,
);
...
const getSystemConfig = async () => {
  try {
    ...
    // 检查status 是不是都为1，都要为1则不显示悬浮提示
    isShowSystemConfig.value = !systemConfig.value.every(
