    ...
    <div class="fixed top-0 z-[889]">
        <div
            ...
            <ElTooltip :content="getWebsiteConfig.shop_title">
                <div class="w-10 h-10 rounded-full border border-[#EDEDED] p-[2px] shrink-0" v-show="!hideSidebar">
                    <router-link to="/" class="flex-shrink-0">
                        <img :src="getLogo" alt="logo" class="w-full h-full rounded-full object-contain" />
                    </router-link>
                </div>
            </ElTooltip>
            <ElTooltip :content="getShopName">
                <div class="text-xs font-medium line-clamp-1 break-all mx-1 cursor-pointer" v-show="!hideSidebar">
                    {{ getShopName }}
                </div>
            </ElTooltip>
...
const { toggleSidebar, getWebsiteConfig, getOemConfig } = useAppStore();
const hideSidebar = computed(() => useAppStore().hideSidebar);

const { is_oem, site_logo, name } = getOemConfig;

const getShopName = computed(() => {
    return is_oem == 1 ? name : getWebsiteConfig.shop_name;
});
