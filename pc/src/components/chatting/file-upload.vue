...
const changeFile = async (e: Event) => {
  ...
  // 上传一开始就同步给父级，避免回车/发送时父级仍以为没有文件或不在上传中
...
const handleUploadFile = async () => {
  ...
  fileLists.value = fileLists.value.filter(
    (item: FileParams) => item.status === UPLOAD_STATUS.SUCCESS
  );
  emit("update:modelValue", fileLists.value);
  emit("change", fileLists.value);
};
...
const submitFileUpload = async (item: FileParams, index: number) => {
  ...
  try {
    ...
    (item as any).id = fileRes.id;
    item.loading = false;
    item.status = UPLOAD_STATUS.SUCCESS;
    // 兼容不同上传接口字段：uri（完整）/ url（相对或完整）
    item.url = fileRes.uri || fileRes.url || item.url;
    fileLists.value[index] = item;
    // 单文件完成后立刻回写父级，避免父级一直拿不到 file_id/url
    emit("update:modelValue", [...fileLists.value]);
    emit("change", [...fileLists.value]);
  } catch (error) {
    // 上传失败或取消时，从请求标识列表中移除
...
watch(
  () => props.modelValue,
  (val: any[] | string) => {
    // 上传进行中时不要被父级 v-model 回写覆盖，否则会丢掉进行中的 reactive 项，导致进度/完成态丢失
    const uploading = fileLists.value.some(
      (item) => item.loading || item.status === UPLOAD_STATUS.UPLOADING
    );
    if (uploading) return;
