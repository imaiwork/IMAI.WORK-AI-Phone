<template>
    <div class="h-full flex items-center px-10 gap-8">
        <div class="flex items-center gap-4">
            <div>
                <ElTooltip content="后退5秒">
                    <div @click="handleTime(-5)">
                        <Icon name="local-icon-replay5" color="var(--color-primary)" :size="24"></Icon>
                    </div>
                </ElTooltip>
            </div>
            <button class="w-9 h-9 flex items-center justify-center bg-primary rounded-full" @click="toggle()">
                <Icon
            ...
            <div>
                <ElTooltip content="快进5秒">
                    <div @click="handleTime(5)">
                        <Icon name="local-icon-neplay5" color="var(--color-primary)" :size="24"></Icon>
                    </div>
                </ElTooltip>
            </div>
        </div>
        <div class="grow relative">
            <div class="absolute -bottom-[20px] left-0 text-[#27264d73] text-xs">
                {{ currentTime }}
            </div>
            <div class="relative py-2 group" @mousemove.stop="getCurrentTime" @click="clickProgress">
                <div class="w-full h-1 relative cursor-pointer progress-bar bg-primary-light-7 group">
                    <div class="absolute top-0 left-0 w-full h-full flex">
                        <div
                            ...
                            <div class="bg-primary-light-6 min-w-[4px] mx-[2px] relative h-full">
                                <div
                                    class="absolute top-0 left-0 h-full bg-primary max-w-full"
                                    :style="{
                                        width: sections.length == 1 ? `${progress}%` : item.width,
                                    }"></div>
                            </div>
                        </div>
                    </div>
                    <div
                        ...
                        <div class="rounded-lg p-4 bg-primary-light-7 min-w-[400px] text-xs">
                            <div class="flex gap-1">
                                <span class="text-[#585A73]">{{ timeTooltip }}</span>
            ...
            <div class="absolute -bottom-[20px] right-0 text-[#27264d73] text-xs">
                {{ duration }}
            </div>
        </div>
        <div class="min-w-[50px]">
            <ElPopover placement="top" popper-class="!w-[105px] !min-w-[90px] !rounded-xl !py-6">
                <template #reference>
...
const emit = defineEmits(["play", "pause", "next", "prev", "end", "update-time"]);
...
const getTotalTime = computed(() => {
    return sections.value.reduce((total, item) => {
...
const getParagraphStyle = (item: any, index: number) => {
    if (item) {
        ...
        const left =
            (sections.value.slice(0, index).reduce((total, item) => {
                return total + (item.End - item.Start);
            }, 0) /
                getTotalTime.value) *
            100;
...
const getCurrentTime = (event: MouseEvent) => {
    ...
    if (progressBar) {
        ...
        const currTime = (x / rect.width) * audio.value.duration;
        currentSection.value =
            sections.value.find((item: any) => {
                return currTime >= item.Start / 1000 && currTime < item.End / 1000;
            }) || {};
