...
export const useUpload = (options: Options) => {
    ...
    const chooseFileCallback = async (filesResult: any) => {
        ...
        if (fileSize > size * 1024 * 1024) {
            feedback.msgWarning(`视频大小不能超过${size}M`);
            return;
        } else if (!isWidthResolutionValid || !isHeightResolutionValid) {
            feedback.msgWarning(
                `上传视频分辨率不能满足${widthResolution[0]}-${widthResolution[1]}x${heightResolution[0]}-${heightResolution[1]}`
            );
            return;
        } else if (!isDurationValid) {
            feedback.msgWarning(`上传视频时长不能小于${duration[0]}秒或大于${duration[1]}秒`);
            return;
        }
        try {
